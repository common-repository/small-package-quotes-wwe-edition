...
class group_small_shipment
{
    ...
    function small_package_shipments($package, $web_service_inst)
    {
        ...
        if (isset($package['contents'])) {
            ...
            $smallpkg_shipping_get_quotes = new smallpkg_shipping_get_quotes();
            $this->destination_Address_wwe_spq = $smallpkg_shipping_get_quotes->destinationAddressWweSmall();
            ...
            foreach ($pack as $item_id => $values) {
                ...
                $ptype = $this->wwe_small_check_product_type($freight_enable_class, $exceedWeight, $product_weight, $en_multiple_package);
                $insurance = $this->en_insurance_checked($values, $_product);
                $product_level_markup = $this->wwe_small_get_product_level_markup($_product, $values['variation_id'], $product_id, $values['quantity']);
                ...
                if($en_pricing_per_product){
                    $pricing_product_origins[$locationId] = $locationId;
                    if ($product_markup > 0 || $product_rental == 'yes') {
                        $locationId = $item_id;
                    }
                }
                ...
                if (!empty($origin_address) && ($product_weight <= $weight_threshold || $en_multiple_package == 'yes')) {
                    // Nested Material
                    ...
                    $hm_plan = apply_filters('wwe_small_packages_quotes_quotes_plans_suscription_and_features', 'hazardous_material');
                    ...
                    if (!$_product->is_virtual()) {
                        ...
                        if($exempt_ground_transit_restriction == 'yes' && !isset($sm_package[$locationId]['exempt_ground_transit_restriction'])){
                            $sm_package[$locationId]['exempt_ground_transit_restriction'] = 1;
                        }
                        ...
                        if (!empty($product_tags)) {
                            ...
                            if (isset($sm_package[$locationId]['product_tags'])) {
                                $sm_package[$locationId]['product_tags'] = array_merge($sm_package[$locationId]['product_tags'], $product_tag_names);
                            } else {
                                $sm_package[$locationId]['product_tags'] = $product_tag_names;
                            }
                        } else {
                            $sm_package[$locationId]['product_tags'] = [];
                        }

                        // Product quantity
                        if (isset($sm_package[$locationId]['product_quantities'])) {
                            $sm_package[$locationId]['product_quantities'] += floatval($values['quantity']);
                        } else {
                            $sm_package[$locationId]['product_quantities'] = floatval($values['quantity']);
                        }

                        // Product price
                        if (isset($sm_package[$locationId]['product_prices'])) {
                            $sm_package[$locationId]['product_prices'] += (floatval($_product->get_price()) * floatval($values['quantity']));
                        } else {
                            $sm_package[$locationId]['product_prices'] = (floatval($_product->get_price()) * floatval($values['quantity']));
                        }
                    }
            ...
            // pricing_per_product
            if(count($pricing_product_origins) > 0){
                $sm_package['pricing_product_origins'] = $pricing_product_origins;
            }
     ...
     * @return string
     */
    function wwe_small_check_product_type($freight_enable_class, $exceedWeight, $weight, $en_multiple_package)
    {
        $weight_threshold = get_option('en_weight_threshold_lfq');
        $weight_threshold = isset($weight_threshold) && $weight_threshold > 0 ? $weight_threshold : 150;
    ...
    function wwe_small_origin_address($values, $_product)
    {
        ...
        if ($enable_dropship == 'yes') {
            ...
            // Multi Dropship
            $multi_dropship = apply_filters('wwe_small_packages_quotes_quotes_plans_suscription_and_features', 'multi_dropship');
        ...
        if (empty($locations_list)) {
            // Multi Warehouse
            $multi_warehouse = apply_filters('wwe_small_packages_quotes_quotes_plans_suscription_and_features', 'multi_warehouse');
    ...
    public function address_array($value)
    {
        ...
        $dropship_data['fee_store_pickup'] = (isset($value->fee_store_pickup)) ? $value->fee_store_pickup : "";
     ...
     */
    function wwe_small_get_product_level_markup($_product, $variation_id, $product_id, $quantity)
    {
