jQuery(document).ready(function () {
    ...
    // Origin terminal address
    // Terminal phone number
    jQuery("#en-phone-number, #en_wd_origin_address, #en_wd_origin_zip , #en_wd_origin_city , #en_wd_origin_state , #en_wd_origin_country, #instore-pickup-address, #instore-pickup-desc, #local-delivery-address, #local-delivery-desc, #local-delivery-fee, #en_wd_origin_markup, #instore-pickup-fee").focus(function (e) {
        jQuery("#add_warehouses #" + this.id).css({'border-color': '#ddd'});
    });

    // Origin terminal address
    // Terminal phone number
    jQuery("#en-phone-number, #en_wd_dropship_nickname, #en_wd_dropship_address, #en_wd_dropship_zip , #en_wd_dropship_city , #en_wd_dropship_state, #en_wd_dropship_country, #instore-pickup-address, #instore-pickup-desc, #local-delivery-address, #local-delivery-desc, #local-delivery-fee, #en_wd_dropship_markup, #instore-pickup-fee").focus(function (e) {
 ...
 * @return {wweSmallinstorePickupInputVal.instorePickup}
 */
if (typeof wweSmallinstorePickupInputVal != 'function') {
    function wweSmallinstorePickupInputVal(formID) {
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_save_warehouse != 'function') {
    function en_wwe_small_wd_save_warehouse() {
        ...
        const enable_instore_pickup = jQuery('#add_warehouses #enable-instore-pickup').is(':checked');
        ...
        switch (true) {
            ...
            case (!(/^[a-zA-Z0-9\-\s]+$/).test(warehouse_en_wd_origin_city)):
                ...
                return false;
            case (enable_instore_pickup && instore_pickup_fee?.length && !((/^([0-9]{1,4})?\.?([0-9]{1,2})$/)).test(instore_pickup_fee)):
                jQuery("#add_warehouses #instore-pickup-fee").parent().find(".en_wd_err").html("Instore pickup fee format should be 100.20 or 10.");
                jQuery("#add_warehouses #instore-pickup-fee").css({ 'border-color': '#e81123' });
                jQuery('.content').delay(200).animate({scrollTop: 0}, 300);
                return false;
            case (enable_local_delivery && local_delivary_fee?.length && !((/^([0-9]{1,4})?\.?([0-9]{1,2})$/)).test(local_delivary_fee)):
                jQuery("#add_warehouses #local-delivery-fee").parent().find(".en_wd_err").html("Local delivery fee format should be 100.20 or 10.");
        ...
        var postForm = {
            'action': 'en_wwe_small_wd_save_warehouse',
        ...
        var arrObj = [postForm, wweSmallinstorePickupInputVal(form_id), localDeliveryInputVal(form_id)];
        ...
        jQuery.ajax({
            ...
            dataType: 'json',
            beforeSend: function () {
                jQuery('.wwe_small_save_warehouse_form').addClass('spinner_disable').val("Loading ..");
            },
            success: function (data) {
                jQuery('.wwe_small_save_warehouse_form').removeClass('spinner_disable').val("Save");
                ...
                if (data.insert_qry == 1) {
                    ...
                    jQuery('tr[id=row_' + WarehpuseDataId + ']').html('<td class="en_wd_warehouse_list_data">' + data.origin_city + '</td><td class="en_wd_warehouse_list_data">' + data.origin_state + '</td><td class="en_wd_warehouse_list_data">' + data.origin_zip + '</td><td class="en_wd_warehouse_list_data">' + data.origin_country + '</td><td class="en_wd_warehouse_list_data"><a href="javascript(0)" title="Edit" onclick="return en_wwe_small_wd_edit_warehouse(' + WarehpuseDataId + ')"><img src="' + script.pluginsUrl + '/small-package-quotes-wwe-edition/warehouse-dropship/wild/assets/images/edit.png"></a><a href="javascript(0)" title="Delete" onclick="return en_wwe_small_wd_delete_current_warehouse(' + WarehpuseDataId + ');"><img src="' + script.pluginsUrl + '/small-package-quotes-wwe-edition/warehouse-dropship/wild/assets/images/delete.png"></a></td>');
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_delete_current_warehouse != 'function') {
    function en_wwe_small_wd_delete_current_warehouse(e) {
        var tab = get_parameter_by_name('tab');
        var postForm = {
            'action': 'en_wwe_small_wd_delete_warehouse',
            'delete_id': e,
            'tab': tab,
        };
        jQuery.ajax({
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_edit_warehouse != 'function') {
    function en_wwe_small_wd_edit_warehouse(e) {
        var postForm = {
            'action': 'en_wwe_small_wd_edit_warehouse',
            'edit_id': e,
        };
        jQuery.ajax({
            type: 'POST',
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_save_dropship != 'function') {
    function en_wwe_small_wd_save_dropship() {
        ...
        const enable_instore_pickup = jQuery('#add_dropships  #enable-instore-pickup').is(':checked');
        ...
        switch (true) {
            ...
            case (!(/^[a-zA-Z0-9\-\s]+$/).test(dropship_en_wd_dropship_city)):
                ...
                return false;
            case (enable_instore_pickup && dropship_instore_pickup_fee?.length && !((/^([0-9]{1,4})?\.?([0-9]{1,2})$/)).test(dropship_instore_pickup_fee)):
                jQuery("#add_dropships #instore-pickup-fee").parent().find(".en_wd_err").html("Instore pickup fee format should be 100.20 or 10.");
                jQuery("#add_dropships #instore-pickup-fee").css({ 'border-color': '#e81123' });
                jQuery('.content').delay(200).animate({scrollTop: 0}, 300);
                return false;
            case (enable_local_delivery && dropship_local_delivary_fee?.length && !((/^([0-9]{1,4})?\.?([0-9]{1,2})$/)).test(dropship_local_delivary_fee)):
                jQuery("#add_dropships #local-delivery-fee").parent().find(".en_wd_err").html("Local delivery fee format should be 100.20 or 10.");
        ...
        var postForm = {
            'action': 'en_wwe_small_wd_save_dropship',
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (data.insert_qry == 1) {
                    ...
                    jQuery('tr[id=row_' + WarehpuseDataId + ']').html('<td class="en_wd_dropship_list_data">' + data.nickname + '</td><td class="en_wd_dropship_list_data">' + data.origin_city + '</td><td class="en_wd_dropship_list_data">' + data.origin_state + '</td><td class="en_wd_dropship_list_data">' + data.origin_zip + '</td><td class="en_wd_dropship_list_data">' + data.origin_country + '</td><td class="en_wd_dropship_list_data"><a href="javascript(0)" title="Edit" onclick="return en_wwe_small_wd_edit_dropship(' + WarehpuseDataId + ')"><img src="' + script.pluginsUrl + '/small-package-quotes-wwe-edition/warehouse-dropship/wild/assets/images/edit.png"></a><a href="javascript(0)" title="Delete" onclick="return en_wwe_small_wd_delete_current_dropship(' + WarehpuseDataId + ');"><img src="' + script.pluginsUrl + '/small-package-quotes-wwe-edition/warehouse-dropship/wild/assets/images/delete.png"></a></td>');
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_edit_dropship != 'function') {
    function en_wwe_small_wd_edit_dropship(e) {
        var postForm = {
            'action': 'en_wwe_small_wd_edit_dropship',
            'dropship_edit_id': e,
        };
        jQuery.ajax({
            type: 'POST',
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_delete_current_dropship != 'function') {
    function en_wwe_small_wd_delete_current_dropship(e) {
        var id = e;
        ...
        jQuery('.confirm_delete').on('click', function () {
            ...
            if (id != false) {
                return en_wwe_small_wd_confirm_delete_dropship(id);
            }
        });
        return false;
    }
 ...
 * @returns {Boolean}
 */
if (typeof en_wwe_small_wd_confirm_delete_dropship != 'function') {
    function en_wwe_small_wd_confirm_delete_dropship(e) {
        var tab = get_parameter_by_name('tab');
        var postForm = {
