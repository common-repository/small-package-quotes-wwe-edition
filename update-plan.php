...
add_action('wp_ajax_en_wwe_small_activate_hit_to_update_plan', 'en_wwe_small_activate_hit_to_update_plan');
add_action('wp_ajax_nopriv_en_wwe_small_activate_hit_to_update_plan', 'en_wwe_small_activate_hit_to_update_plan');

/**
 * Activate WWE SMALL
 */
function en_wwe_small_activate_hit_to_update_plan($network_wide = null)
{
    if ( is_multisite() && $network_wide ) {
        ...
        foreach (get_sites(['fields'=>'ids']) as $blog_id) {
            ...
            $url = WWE_DOMAIN_HITTING_URL . "/web-hooks/subscription-plans/create-plugin-webhook.php?";
            ...
            update_option('wwe_small_packaging_type', $packaging_type);

            en_check_wwe_small_plan_on_product_detail();
...
function en_check_wwe_small_plan_on_product_detail()
{
    ...
    // Nested material
    $nested_mateials = apply_filters('wwe_small_packages_quotes_quotes_plans_suscription_and_features', 'nested_material');
...
//  Dropship
    if (get_option('wwe_small_packages_quotes_store_type') == "1") {
        $action_dropship = apply_filters('wwe_small_packages_quotes_quotes_plans_suscription_and_features', 'multi_dropship');
 ...
 */
function en_wwe_small_deactivate_hit_to_update_plan($network_wide = null)
{
    if ( is_multisite() && $network_wide ) {
 ...
 * @return string
 */
function en_wwe_small_plan_name()
{
 ...
 * @return string
 */
function en_wwe_small_plan_notice()
{
    if (isset($_GET['tab']) && ($_GET['tab'] == "wwe_small_packages_quotes")) {
        ...
        if ($store_type == "1" || $store_type == "0" && ($plan_number == "0" || $plan_number == "1" || $plan_number == "2" || $plan_number == "3")) {
            ...
            $click_here_to_update_plan = ' <a href="javascript:void(0)" data-action="en_wwe_small_activate_hit_to_update_plan" onclick="en_update_plan(this);">Click here</a> to refresh the plan';

            $plan_package = en_wwe_small_plan_name();
...
add_action('admin_notices', 'en_wwe_small_plan_notice');
